...
struct probe_sampletab pmacnet_cols[] = {
...
struct probe_rowdiff pmacnet_diffs[] = {
     PROBE_ENDROWDIFF
};

/* static data return methods */
struct probe_sampletab *pmacnet_getcols()    {return pmacnet_cols;}
struct probe_rowdiff   *pmacnet_getrowdiff() {return pmacnet_diffs;}
char                  **pmacnet_getpub()     {return NULL;}

/* linux version; assume 2.4 being the latest */
int pmacnet_linuxversion=24;
int pmacnet_ndev=1;
 ...
 */
void pmacnet_col_netdev(TABLE tab, ITREE *idata) {
     ...
     if (pmacnet_linuxversion == 24 || pmacnet_linuxversion == 22 || 
	 pmacnet_linuxversion == 26) {
	  itree_first(idata);
...
void pmacnet_derive(TABLE prev, TABLE cur) {}
