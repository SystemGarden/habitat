/*
 * Linux downtime probe for habitat
 *
...
char *plindown_purl_boot=NULL;
char *plindown_purl_alive=NULL;
char *plindown_usage = "down <boot> <alive>\n"
...
struct probe_sampletab plindown_cols[] = {
...
struct probe_rowdiff plindown_diffs[] = {
     PROBE_ENDROWDIFF
};

/* static data return methods */
struct probe_sampletab *plindown_getcols()    {return plindown_cols;}
struct probe_rowdiff   *plindown_getrowdiff() {return plindown_diffs;}
char                  **plindown_getpub()     {return NULL;}
...
void plindown_init(char *probeargs) {
     ...
     plindown_purl_boot  = xnstrdup(boot);
     plindown_purl_alive = xnstrdup(alive);
}
...
void plindown_collect(TABLE tab) {
     ...
     if (plindown_purl_boot == NULL || plindown_purl_alive == NULL) {
     ...
     if (bootstr == NULL) {
	  ...
	  boot = plindown_stampboot(plindown_purl_boot);
	  elog_printf(DIAG, "No 'boot' timestamp at %s: "
		      "stamping now boot=%d", plindown_purl_boot, boot);
     } else {
          boot = strtol(bootstr, NULL, 10);
     }

     alivestr = route_read(plindown_purl_alive, NULL, &len);
     if (alivestr == NULL) {
	  /* No alive timestamp, which we rely on others to maintain
	   * in order to calculate an accurate down time.
	   * Give the current time. */
	  alive = plindown_stampalive(plindown_purl_alive);
	  elog_printf(DIAG, "No 'last alive' timestamp at %s: "
		      "stamping now alive=%d", plindown_purl_alive, alive);
     } else {
     ...
     if (boot > alive) {
	  ...
	  plindown_stampboot (plindown_purl_boot);
	  plindown_stampalive(plindown_purl_alive);
     }
...
void plindown_fini()
{
     if (plindown_purl_boot)
	  nfree(plindown_purl_boot);
     if (plindown_purl_alive)
	  nfree(plindown_purl_alive);
}
...
int plindown_stampboot(char *boot_purl)
{
     ...
     if (plindown_getutmpuptime(&down, &boot, "/var/run/utmp")) {
