/*
 * Mac OS X uptime probe for iiab
...
struct probe_sampletab pmacup_cols[] = {
...
struct probe_rowdiff pmacup_diffs[] = {
     PROBE_ENDROWDIFF
};

/* static data return methods */
struct probe_sampletab *pmacup_getcols()    {return pmacup_cols;}
struct probe_rowdiff   *pmacup_getrowdiff() {return pmacup_diffs;}
char                  **pmacup_getpub()     {return NULL;}
...
/* interpret the data as an uptime format and place it into pmacup_tab */
void  pmacup_col_uptime(TABLE tab, char *data)
{
     ...
     if (access("/var/run/utmp", R_OK) == 0) {
	  ...
	  if (pmacup_getutmpuptime(&down, &boot, "/var/run/utmp"))
 ...
 * current row of  pmacup_tab */
void  pmacup_col_cpuinfo(TABLE tab, char *data)
{
...
int pmacup_getutmpuptime(time_t *down, time_t *boot, char *filename)
{
...
void pmacup_derive(TABLE prev, TABLE cur) {}
