...
struct probe_sampletab pmacintr_cols[] = {
...
struct probe_rowdiff pmacintr_diffs[] = {
     PROBE_ENDROWDIFF
};

/* static data return methods */
struct probe_sampletab *pmacintr_getcols()    {return pmacintr_cols;}
struct probe_rowdiff   *pmacintr_getrowdiff() {return pmacintr_diffs;}
char                  **pmacintr_getpub()     {return NULL;}

/* linux version; assume 2.4 being the latest */
int pmacintr_linuxversion=24;
int pmacintr_ncpu=1;
 ...
 */
void pmacintr_collect(TABLE tab) {
     char *data;
     ITREE *lines;
     ...
     if (data) {
	  ...
	  pmacintr_ncpu = itree_n((ITREE *)itree_get(lines));
	  itree_traverse(lines) {
	       if (itree_isatstart(lines))
		    continue;
	       table_addemptyrow(tab);
	       pmacintr_col_intr(tab, itree_get(lines) );
	  }
...
void pmacintr_col_intr(TABLE tab, ITREE *idata) {
      ...
      * These are taken to fill the table pmacintr_tab, columns name and hard.
      *
     ...
     } else if (pmacintr_linuxversion == 24 || pmacintr_linuxversion == 26) {
          int i, val=0;

	  for (i=1; i <= pmacintr_ncpu; i++) {
	       if (itree_find(idata, i) == ITREE_NOVAL)
		    return;
	       val += strtol(itree_get(idata), NULL, 10);
	  }

	  if (itree_n(idata) == pmacintr_ncpu + 1)
...
void pmacintr_derive(TABLE prev, TABLE cur) {}
...
main(int argc, char *argv[]) {
     ...
     pmacintr_collect();
     if (argc > 1)
	  buf = table_outtable(pmacintr_tab);
     else
	  buf = table_print(pmacintr_tab);
     puts(buf);
     nfree(buf);
     table_destroy(pmacintr_tab);
     exit(0);
}
