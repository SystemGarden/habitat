...
#define PMACPS_STATSZ 256

ITREE *pmacps_uidtoname;	/* uid to username lookup */
int    pmacps_pagesize;		/* pagesize in bytes */
float  pmacps_pagetokb;		/* factor to multiply pages to get kb */
time_t pmacps_boot_t;		/* time system booted */
long   pmacps_total_mem;	/* total memory size */
time_t pmacps_filter_t;		/* time stamp of filter route */
char * pmacps_filter_purl;	/* p-url of the filter */
char * pmacps_filter_cmds;	/* table of filter commands */
TABSET pmacps_filter_tset;	/* compiled table set instance */

/* table constants for system probe */
struct probe_sampletab pmacps_cols[] = {
...
struct probe_rowdiff pmacps_diffs[] = {
  PROBE_ENDROWDIFF,
     {"pc_cpu",  "pc_cpu_diff"}
};

/* static data return methods */
struct probe_sampletab *pmacps_getcols()    {return pmacps_cols;}
struct probe_rowdiff   *pmacps_getrowdiff() {return pmacps_diffs;}
char                  **pmacps_getpub()     {return NULL;}

/* prototypes */
void pmacps_load_filter(char *probeargs);
void pmacps_compile_filter(TABLE tab);
 ...
 */
void pmacps_init(char *probeargs) {

     /* set filter parameters and carry out initial load */
     pmacps_filter_t    = (time_t) 0;
     pmacps_filter_purl = NULL;
     pmacps_filter_cmds = NULL;
     pmacps_filter_tset = NULL;
     pmacps_load_filter(probeargs);

     /* create the uid-to-name user list */
     pmacps_uidtoname = itree_create();

     /* conversion factors for pages */
     pmacps_pagesize = getpagesize();
     pmacps_pagetokb = pmacps_pagesize / 1024;

     /* boot time */
     pmacps_boot_t = pmacps_getboot_t();

     /* elog_printf(DEBUG, "pmacps_pagesize = %d, pmacps_pagetokb = %f, "
	                   "pmacps_getboot_t = %d", pmacps_pagesize, 
			   pmacps_pagetokb, pmacps_boot_t);*/

     /* get memory size */
     pmacps_total_mem = pmacps_gettotal_mem();
}


/* destroy any structures that may be open following a run of sampling */
void pmacps_fini() {
     itree_clearoutandfree(pmacps_uidtoname);
     itree_destroy(pmacps_uidtoname);
     if (pmacps_filter_purl)
          nfree(pmacps_filter_purl);
     if (pmacps_filter_cmds)
          nfree(pmacps_filter_cmds);
     if (pmacps_filter_tset)
          tableset_destroy(pmacps_filter_tset);
}
...
void pmacps_load_filter(char *probeargs) {
     ...
     if (probeargs && *probeargs) {
	  ...
	  /* hold new location and force load by resetting timestamp */
          if (pmacps_filter_purl)
	       nfree(pmacps_filter_purl);
	  pmacps_filter_purl = xnstrdup(filter_purl);
	  pmacps_filter_t = (time_t) 0;
          elog_printf(DIAG, "new ps probe filter '%s'", pmacps_filter_purl);
     }
     ...
     pmacps_filter_cmds = filter_cmds;
}



/*
 * Compile the filter from commands brought in by pmacps_load_filter()
 */
void pmacps_compile_filter(TABLE tab) {
     int r;
     ...
     pmacps_filter_tset = tableset_create(tab);
     r = tableset_configure(pmacps_filter_tset, pmacps_filter_cmds);
     if ( ! r ) {
...
void pmacps_collect(TABLE tab) {
     ...
     while ((d = readdir(dir))) {
	  ...
	  if (data) {
	       ...
	       /*pmacps_col_fperm(tab, pfile, pmacps_uidtoname);*/
	       pmacps_col_stat(tab, data, pmacps_uidtoname);
	  ...
	  data = probe_readfile(pfile);
	  if (data) {
	       pmacps_col_status(tab, data, pmacps_uidtoname);
	       table_freeondestroy(tab, data);
	  }
	  ...
	  data = probe_readfile(pfile);
	  if (data) {
	       pmacps_col_statm(tab, data, pmacps_uidtoname);
	       table_freeondestroy(tab, data);
	  }
     ...
     pmacps_load_filter(NULL);
     pmacps_compile_filter(tab);
     if (pmacps_filter_tset) {
          /* filter the main table into a subset, then replace the original
	   * with the subset */
          filtered_tab = tableset_into(pmacps_filter_tset);
	  table_rmallrows(tab);
...
void pmacps_col_fperm(TABLE tab, char *fname, ITREE *uidtoname)
{
     struct stat buf;
     ...
     if (stat(fname, &buf))
	  ...
	  table_replacecurrentcell_alloc(tab, "gid", util_i32toa(buf.st_gid));
     	  table_replacecurrentcell(tab, "pwname", 
				   pmacps_getuser(buf.st_uid, uidtoname));
     }
}

/* find the full command line */
void pmacps_col_cmd(TABLE tab, char *value)
{
...
void pmacps_col_stat(TABLE tab, char *ps, ITREE *uidtoname)
{
     char *value, *value2;
     ...
     value = strtok(NULL, " ");
     runstart = pmacps_boot_t + (strtol(value, NULL, 10) / 100);
...
void pmacps_col_status(TABLE tab, char *ps, ITREE *uidtoname)
{
     ITREE *lines, *cols;
     char *key, *val;
     ...
     itree_traverse(lines) {
	  ...
	  case 'U':	/* Uid */
	       if (strcmp(key, "Uid") == 0) {
		    ...
		    table_replacecurrentcell(tab, "pwname", 
			pmacps_getuser(strtol(val, NULL, 10), uidtoname));
...
void pmacps_col_statm(TABLE tab, char *ps, ITREE *uidtoname)
{
     char *value;
...
char *pmacps_getuser(int uid, ITREE *uidtoname)
{
     char *name;
     struct passwd *pwent;
...
void pmacps_derive(TABLE prev, TABLE cur) {}
...
main(int argc, char *argv[]) {
     ...
     pmacps_collect();
     if (argc > 1)
	  buf = table_outtable(pmacps_tab);
     else
	  buf = table_print(pmacps_tab);
     if (buf)
	  puts(buf);
     else
	  elog_die(FATAL, "pmacps", 0, "no output produced");
     nfree(buf);
     table_destroy(pmacps_tab);
     pmacps_fini();
     iiab_stop();
     exit(0);
}
