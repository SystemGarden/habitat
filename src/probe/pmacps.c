...
PROCESS TABLE STRUCTURE IS DEFINED IN KINFO_PROC IN SYS/SYSCTL.H

/*
 */
ITREE *pmacps_uidtoname;
...
/*
 * Initialise probe for macaris system information
 */
void pmacps_init() {
...
void pmacps_collect(TABLE tab) {
     ...
     while ( (d = readdir(dir)) ) {
	  /* the macaris /proc only contains pids and dot files 
	   * -- nothing else */
	  ...
	  if (data) {
	       ...
	       pmacps_col_psinfo(tab, data, pmacps_uidtoname);
	       nfree(data);
	  } else
	       continue;
     ...
     closedir(dir);
     /*itree_clearoutandfree(pmacps_uidtoname); TODO: destroy only when probe is*/
}

/* takes data from /procs's psinfo structure into the table */
void pmacps_col_psinfo(TABLE tab, psinfo_t *ps, ITREE *uidtoname)
{
     char *value;
     ...
     table_replacecurrentcell_alloc(tab, "pwname",    pmacps_getuser(ps->pr_uid, 
							       uidtoname));
     table_replacecurrentcell_alloc(tab, "euid",      util_u32toa(ps->pr_euid));
     table_replacecurrentcell_alloc(tab, "epwname",   pmacps_getuser(ps->pr_euid, 
							       uidtoname));
...
void pmacps_col_usage(TABLE tab, prusage_t *pu)
{
...
void pmacps_col_status(TABLE tab, pstatus_t *pu)
{
     table_replacecurrentcell_alloc(tab, "pendsig",  
			      pmacps_getsig(&pu->pr_sigpend));
