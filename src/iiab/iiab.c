...
int  iiab_ispidrunning(int pid) {
     ...
     /*
      * this code uses the /proc filesystem and works on linux and solaris only 
      */
     char fname[20];
...
/* Save or update to a file (route) the configuration entry identified 
 * by (cf,key), being the configuration class and the key name.
 * The file to which to save the data is identifed by (cf,IIAB_CFUSERKEY)
 * held in the same configuration class and will be headed with 
 * IIAB_CFUSERMAGIC using cf_updateline(). 
 * IIAB_CFUSERKEY and IIAB_CFUSERMAGIC are both a #define'ed.
 * Returns the number of characters read or -1 for error */
