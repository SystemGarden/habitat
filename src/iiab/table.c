...
void table_destroy(TABLE t)
{
     if (t == NULL)
	  return;
     if ( t->refcount == 0 ) {
          elog_printf(DEBUG, "Destroying a table with no reference count");
	  return;
     }
     if (--t->refcount > 0)
	  return;
 ...
 * data will be referred to and will be modified in the scanning process. 
 ...
 * The list should be free'ed with tree_destroy() but the data left intact
 * as they are not a copy.
 */
 ...
 * if uniq!=NULL, then uniq will be returned, so there is no additional data
 * to manage (or free).
 */
