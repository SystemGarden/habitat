	       ...
	       char * state_purl	/* purl to store state */ )
{
     ROUTE state_rt, rt;
     TABLE state, io;
     int r, local_seq, remote_seq, local_max_seq;
...
#define TSTATEPURL1 "grs:" TFILE1 "," TSTATE1 ",0"
...
int main(int argc, char **argv) {
     ...
     route_expand(buf4, "grs:rep.%h.rs,rita>sqlrs:%h,rita", "NOJOB");
     route_expand(buf5, "grs:rep.%h.rs,sue>sqlrs:%h,sue", "NOJOB");
     route_expand(buf6, "grs:rep.%h.rs,bob>sqlrs:%h,bob", "NOJOB");
