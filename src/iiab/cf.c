 ...
 * If false scanned data will be ignored for that key.
 * cf_scan() always attempts to recover from parsing errors.
	    ...
	    int overwrite	/* overwrite existing data? */ )
{
     ...
     itree_traverse(lol) {
	  ...
	  case 1:
	       ...
	       switch (* ((char *) itree_get( itree_get(lol) )) ) {
	       case '-':
		    if ( ! overwrite && 
			     tree_find(cf, itree_get(lol)+1) != TREE_NOVAL ) {
			 nfree(entry);
			 continue;	/* don't overwrite existing data */
		    }
		    entry->data.arg = xnstrdup("-1");
		    cf_entreplace(cf, 
			       xnstrdup( ((char *) itree_get(itree_get(lol)) )
			       +1), entry);
		    nfree(itree_get(itree_get(lol)));
		    break;
	       case '+':
		    if ( ! overwrite && 
			     tree_find(cf, itree_get(lol)+1) != TREE_NOVAL ) {
	       ...
	       break;
	  case 2:
	       /* value assignmant -- take over parenting of first two args */
	       if ( ! overwrite && 
		           tree_find(cf, itree_get(lol)) != TREE_NOVAL )
		    continue;	/* don't overwrite existing data */
	       entry = xnmalloc(sizeof(struct cf_entval));
	       ...
	       break;
	  default:
	       /* value -- reparent the argument ITREE */
	       if ( ! overwrite && 
		          tree_find(cf, itree_get(lol)) != TREE_NOVAL )
		    continue;	/* don't overwrite existing data */
	       itree_first( itree_get(lol) );
	       ...
	       cf_entreplace(cf, xnstrdup( itree_get(itree_get(lol)) ), entry);
	       itree_rm(itree_get(lol));
	       break;
	  }
     }
 ...
 * Key will be reparented and so the caller should not manage its space.
...
/*
 * Add or replace the value with an ITREE newval.
 * The storage in newval will be reparented, so it should be an 
 * independently allocated amount of storage that may be nfree()ed
 * automatically.
 */
 ...
 * Storage is not malloc()ed to store values; all the values are expected to
 * exist for the duration of the cf list.
 ...
 * Returns the number of insertions made.
 */
int cf_defaultcf(CF_VALS cf,		/* Parsed configuration values */
