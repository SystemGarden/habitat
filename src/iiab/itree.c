 ...
 */
ITREE *itree_really_create(char *rfile, int rline, const char *rfunc) {
	ITREE *t;

	/* allocate using normal malloc but then ask nmalloc to track
	 * ITREE top level allocation as a first class object, showing 
	 * the actuall caller */
	t = malloc(sizeof(ITREE));
	if (nm_active)
	     nm_add(NM_ITREE, t, sizeof(ITREE), rfile, rline, rfunc);
 ...
 */
void itree_really_destroy(ITREE *t, char *rfile, int rline, const char *rfunc) {
	if (t == NULL) {
	...
	if (nm_active)
	     nm_rm(NM_ITREE, t, rfile, rline, rfunc);
	free(t);
}
...
/*
 * Duplicate an ITREE containing a list of strings. No shared memory in
 * the duplicate, which is passed back in the return.
 * If input ITREE is NULL, NULL will be returned in turn.
 */
ITREE *itree_copystr(ITREE *in)
{
	ITREE *out;
	...
	out = itree_create();
	itree_traverse(in)
	     itree_add(out, itree_getkey(in), xnstrdup(itree_get(in)));
