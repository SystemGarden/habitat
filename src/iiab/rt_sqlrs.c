...
const struct route_lowlevel rt_sqlrs_method = {
     ...
     rt_sqlrs_tread,      rt_sqlrs_status,     rt_sqlrs_checkpoint
};
CF_VALS rt_sqlrs_cf;
...
/* Checkpoint is not yet applicable here */
int    rt_sqlrs_checkpoint (RT_LLD lld)
{
     RT_SQLRSD rt;
			      ...
			      char **cookiejar	/* returned cookiejar fname */)
{
  char *auth_purl, auth_expanded_purl[1024], *authtxt, *config_file;
	  ...
	  *cookiejar = NULL;
	  ...
	  if (auth_purl == NULL) {
	       elog_printf(DIAG, "No configuration found for authorisation: "
			   "%s, proceeding without authorisation for %s", 
	  ...
	  if (cookies_purl == NULL) {
	       ...
	       elog_printf(DIAG, "No configuration found for cookies: %s "
	  ...
	  if (cookiejar_unexpanded == NULL) {
	       elog_printf(DIAG, "No configuration found for cookie jar: %s, "
