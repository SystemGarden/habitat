 ...
 * The returned text should be nfree()ed after use
 */
	       ...
	       int     flags	/* flags */ )
{
      ...
      * Note on buf memory - it is allocated by http_receive using nrealloc
      * so it does not need to be specifically adopted */
     ...
     if (r) {
          /* if there was an error, discard the text and die */
          if (buf.memory)
	       nfree(buf.memory);	/* my http_recieve allocates with nm */
	  return NULL;
     }
