...
/*
 * Return a string which will always be scanned as a single token.
...
/* Get the file extension from path, defined as the trailing string after
 * the last full stop '.'. Returns a pointer into the existing string */
 ...
 * Warning: single threaded function, returns a static
 */
...
/*
 * Generate a shorter ascii representation of time
 * Warning: single threaded function, returns a static
 */
char *util_sdecdatetime(time_t t)
{
     static char buf[UTIL_SHORTSTR];
     struct tm *time;

     time = localtime(&t);
     strftime(buf, UTIL_SHORTSTR, "%d-%b-%y %I%p", time);
...
/*
 * Generate a short ascii representation of time, adapting its format
 * to the distance between two events t and t_ref: the further away in time,
 * the more approximate the representation.
 * Warning: single threaded function, returns a static */
char *util_shortadaptreldatetime(time_t t, time_t ref_t) {
     static char buf[UTIL_SHORTSTR];
     struct tm entry, ref;

     memcpy(&entry, localtime(&t), sizeof(struct tm));
     memcpy(&ref, localtime(&ref_t), sizeof(struct tm));

     strftime(buf, UTIL_SHORTSTR, "%c", &entry); /* also %d-%b-%y %I:%M:%S %p*/
     /* Or adaptive  recent: %I:%M:%S */
     /* longer term: %d%b */

     if (entry.tm_year != ref.tm_year)
	  /* not this year */
	  if ( abs(entry.tm_year - ref.tm_year) <= 1 &&
	       abs(entry.tm_mon  - ref.tm_mon) < 6 )
	       /* ...but less than six months adjacent, give date */
	       strftime(buf, UTIL_SHORTSTR, "%d %b", &entry);
	  else
	       /* all other times, give year */
	  ...
	  /* this year */
	  if (entry.tm_yday != ref.tm_yday)
	       /* not today */
	       if ( abs(entry.tm_yday - ref.tm_yday) <= 1 && 
		    abs(entry.tm_hour - ref.tm_hour) <= 12 )
		    /* ...but less than 12 hours adjacent, give hours */
		    strftime(buf, UTIL_SHORTSTR, "%H:%M:%S", &entry);
	       else
		    /* Older than 12 hours, give date */
...
/*
 * Return the approximate difference between two times as a string 
 * containing English in lowercase. The string should be freed with nfree().
 */
char *util_approxtimedist(time_t oldest, time_t youngest)
{
     long tdiff;
     char str[101];

     tdiff = (long) difftime(youngest, oldest);
     if (tdiff < 0) {
          tdiff = labs(tdiff);
     }
     if (tdiff == 0) {
          snprintf(str, 100, "no difference");
     } else if (tdiff >= 63072000) {
          /* over 2y */
          snprintf(str, 100, "%lu years", tdiff / 31536000);
     } else if (tdiff >= 34128000) {
          /* over 1y 1m */
          snprintf(str, 100, "one year %lu months", 
		   (tdiff-31536000) / 2592000);
     } else if (tdiff >= 31536000) {
          /* over 1y */
          snprintf(str, 100, "one year");
     } else if (tdiff >= 5184000) {
          /* over 2M */
          snprintf(str, 100, "%lu months", tdiff / 2592000);
     } else if (tdiff >= 2592000) {
          /* over 1M */
          snprintf(str, 100, "one month");
     } else if (tdiff >= 172800) {
          /* over 2d */
          snprintf(str, 100, "%lu days", tdiff / 86400);
     } else if (tdiff >= 86400) {
          /* over 1d */
          snprintf(str, 100, "one day");
     } else if (tdiff >= 7200) {
          /* over 2h */
          snprintf(str, 100, "%lu hours", tdiff / 3600);
     } else if (tdiff >= 3600) {
          /* over 1d */
          snprintf(str, 100, "one hour");
     } else if (tdiff >= 120) {
          /* over 2m */
          snprintf(str, 100, "%lu minutes", tdiff / 60);
     } else if (tdiff >= 60) {
          /* over 1m */
          snprintf(str, 100, "one minute");
     } else {
          snprintf(str, 100, "less that a minute");
     }
