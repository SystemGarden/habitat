...
char *httpd_builtin_cf(char *path, int match, int method, TREE *headers_in, 
     ...
     /* if the url ends in tsv, flag for printing in tab separated format */
     if (strlen(path) > 3 && strncmp(path+strlen(path)-3, "tsv", 3) == 0)
	  tsv++;
 ...
 * '/localtsv' may be substuted for '/local', which is special measure to
...
char *httpd_builtin_local(char *path, int match, int method, TREE *headers_in, 
     ...
     /* if the first part of the url (first directory) ends in tsv, flag 
      * for printing in tab separated format */
     if (strcspn(path+1, "/")+1 > 3 && strncmp(path+strcspn(path+1, "/")+1-3,
					       "tsv", 3) == 0)
	  tsv++;

     /* work out the file to open */
     /* TODO: This needs work. we are hard coding the driver [grs:] but
      * we should be working this out dynamically */
     rspath    = util_strjoin("grs:", iiab_dir_var, "/%h.grs", 
			      *r=='?' ? "": ",", r, NULL);
