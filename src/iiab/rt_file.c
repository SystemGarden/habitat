...
const struct route_lowlevel rt_filea_method = {
     ...
     rt_file_tread,      rt_file_status,     rt_file_checkpoint
};
...
int    rt_file_access(char *p_url, char *password, char *basename, int flag)
{
     ...
     if ((flag & ROUTE_READOK) && (access(basename, R_OK) == 0))
	  return 1;
     else if ((flag & ROUTE_WRITEOK) && (access(basename, W_OK | R_OK) == 0))
	  return 1;
     else
	  return 0;
}
     ...
     if (keep > 0) {
	  ...
	  if (fd == -1) {
	       if (access(basename, (R_OK |W_OK)) == 0) {
		    ...
		    if (fd == -1) {
		         elog_printf(FATAL, "should be able to open "
				     "file %s for writing but can't", 
				     basename);
			 return NULL;
		    }
	       } else {
		    elog_printf(FATAL, "can't open file %s for writing", 
				basename);
		    return NULL;
	       }
	  }
          ...
          if (access(basename, R_OK) == 0) {
	       ...
	       if (fd == -1) {
		    elog_printf(FATAL, "should be able to open "
				...
				"append method as keep lt 0)", basename);
		    return NULL;
	       }
	  } else if (access(basename, F_OK) == 0) { 
	       elog_printf(FATAL, "file % exists but no read permission "
			   "(reading in append method as keep lt 0)",basename);
	       return NULL;
	  } else {
	       elog_printf(FATAL, "file %s does not exist for reading; "
 ...
 * a timestamp is in a column called '_time'. No parsing will be carried out */
TABLE rt_file_tread  (RT_LLD lld, int seq, int offset)
{
     ...
     /* Seek and allocate buffer space with allowance for a NULL */
     lseek(rt->fd, offset, SEEK_SET);
     ...
     *(buf+buflen) = '\0';
     ...
     table_replacecurrentcell_alloc(tab, "_time", util_i32toa(sbuf.st_mtime));
...
/*
 * Checkpoint is simple, just flush
 */
int    rt_file_checkpoint (RT_LLD lld)
{
     RT_FILED rt;
     int r;
