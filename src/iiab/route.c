 ...
 * Read or write flags qualify how the file is accessed.
 * If asking for write access, many ROUTE types will also check for
 * read access if they are needed by the type.
 * Returns 1 for access, 0 for no access
 */
int route_access(char *p_url,	/* Pseduo-url */
		 char *password,/* Password if supported by channel */
		 int flags	/* ROUTE_READOK or ROUTE_WRITEOK */ )
{
     ROUTE_METHOD meth;
     char *base;
 ...
 * If keep is 0, then new routes will not be created and some methods
 * won't write.
		   ...
		   int   interval	/* Interval in seconds */ )
{
     ...
     route_expand(buf, p_url, jobname, interval);
		 ...
		 int   interval	/* %i value - interval */ )
{
     ...
     while ((pt = strchr(pt, '%'))) {
	  ...
	  case 'f':	/* %f - Insert fully qualified host name */
	       ...
	       bpt += l;
	       break;
	  case 'i':	/* %i - Insert interval */
	       bpt += sprintf(bpt, "%d", interval);
	       break;
...
/*
 * Perform regular storage maintenance
 * This is a periodic reorganisation, clear up or checkpoint of the storage
 * triggered when there is thought to be time to do so.
 * Whilst a single ROUTE is supplied, maintenance should be applied to
 * the whole storage container.
 * Returns 1 if successful, 0 otherwise such as the route not existing.
 */
int route_checkpoint(ROUTE rt		/* route */ )
{
     if (!rt)
	  return 0;
     
     return rt->method->ll_checkpoint(rt->handle);
}
