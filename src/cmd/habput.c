...
"where <route>     destination route (enhanced url)\n"
...
int main(int argc, char *argv[]) {
     ...
     /* open route */
     out = route_open(cf_getstr(iiab_cmdarg, "argv1"), 
		      cf_getstr(iiab_cmdarg, "m"), 
		      cf_getstr(iiab_cmdarg, "p"),
		      cf_getint(iiab_cmdarg, "n"));
     if ( ! out ) {
          elog_send(FATAL, "unable to open route");
	  iiab_stop();
	  exit(1);
     }
     ...
     while ( (nread = read(0, buf, PIPE_BUF)) > 0) {
	  ...
	  if (r == -1)
	       elog_die(FATAL, "error writing to route");
     }
     if (readtext)
	  route_flush(out);
     else {
	  ...
	  if (r == -1) {
	       elog_send(FATAL, "unable to scan into table");
	  } else {
	       r = route_twrite(out, tab);
	       if ( !r )
		    elog_send(FATAL, "unable to write table to route");
	  }
	  table_destroy(tab);
     }
